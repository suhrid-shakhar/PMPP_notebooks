...
__global__ void private_histo_kernel(char *data, unsigned int length, unsigned int *histogram)
{
    ...
    if ( i < length)
    {
        ...
        {
            atomicAdd(&histogram[blockIdx.x * NUM_BINS + alphabet_position/4], 1);
        }
    }
    if( blockIdx.x > 0)
    {
        ...
        {
            unsigned int binValue = histogram[blockIdx.x * NUM_BINS + bin];
            if(binValue > 0) 
            {
...
int main(int argc, char* argv[])
{
    ...
    cudaCheck(cudaMemset(cpu_histogram_h, 0, histoSize));
